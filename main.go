...
import (
	...
	"git.xenonstack.com/akirastack/continuous-security-auth/src/routes"
)
...
func main() {
	...
	postgreqlDB, err := db.DB()
	// close db instance whenever whole work completed
	postgreqlDB.SetMaxIdleConns(ideal)
	postgreqlDB.SetConnMaxLifetime(1 * time.Hour)
	config.DB = db
	// close db instance whenever whole work completed
...
func Dummy() {
	data := database.Accounts{}
	data.Userid = 50
	data.Name = "Rahul Kumar"
	data.Email = "rahul@xenonstack.com"
	data.RoleID = "admin"

	D := jwtToken.JwtToken(data)
	log.Println(D)
}
